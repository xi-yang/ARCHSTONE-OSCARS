 ...
 *
 */
@OSCARSNetLoggerize(moduleName = ModuleName.PSS_ARCH)
@OSCARSService (
        implementor = "net.es.oscars.pss.soap.gen.PSSService",
        namespace   = "http://oscars.es.net/OSCARS/pss",
        serviceName = ServiceNames.SVC_PSS_ARCH
)
public class ArchPSSClient extends OSCARSSoapService<PSSService, PSSPortType>  {
    static private Logger LOG = Logger.getLogger(ArchPSSClient.class);

    private ArchPSSClient (URL host, URL wsdlFile) throws OSCARSServiceException {
        super (host, wsdlFile, PSSPortType.class);
    }

    static public ArchPSSClient getClient (URL host, URL wsdl)
        ...
        try {
            ...
            LOG.error(netLogger.error("ProxyPSSClient.getClient", ErrSev.MAJOR, " caughtException: " + e.getMessage()));
            e.printStackTrace();
            throw new OSCARSServiceException(e.getMessage());
        }
        ArchPSSClient client = new ArchPSSClient (host, wsdl);
        return client;
    }
