 ...
 *
 */
@OSCARSNetLoggerize(moduleName = ModuleName.PSS_DRAGON)
@OSCARSService (
...
public class DragonPSSClient extends OSCARSSoapService<PSSService, PSSPortType>  {
    static private Logger LOG = Logger.getLogger(DragonPSSClient.class);

    private DragonPSSClient (URL host, URL wsdlFile) throws OSCARSServiceException {
        super (host, wsdlFile, PSSPortType.class);
    }

    static public DragonPSSClient getClient (URL host, URL wsdl)
        ...
        try {
            ...
            LOG.error(netLogger.error("DragonPSSClient.getClient", ErrSev.MAJOR, " caughtException: " + e.getMessage()));
            e.printStackTrace();
            throw new OSCARSServiceException(e.getMessage());
        }
        DragonPSSClient client = new DragonPSSClient (host, wsdl);
        return client;
    }
